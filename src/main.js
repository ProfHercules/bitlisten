...
$(document).ready(function () {
  ...
  debugSpawner = function () {
    ...
    if (Math.random() <= 0.1) {
      ...
      if (Math.random() < 0.5) new Transaction(volume, false);
      else new Transaction(volume, false, volume * 75, "USD");
    }
  };
...
var showChart = function () {
  ...
  if ($("#bitcoinChart").length === 0) {
    // Load the iframe
    $("#chartHolder").html(
      '<iframe id="bitcoinChart" scrolling="no" frameBorder="0" src="http://bitcoin.clarkmoody.com/widget/chart/zeroblock/"></iframe>'
    );
  }
  $("#pageSplitter").colResizable({
    liveDrag: true,
