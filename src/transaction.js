...
function Transaction(bitcoins, highlight, currency, currencyName) {
  if (document.visibilityState === "visible") {
    ...
    var bitcoinVal = Number(bitcoins).toLocaleString(undefined, {
    ...
    if (globalShowDollar === true) {
      bitcoinString = `\$ ${Number(bitcoins * globalRate).toLocaleString(
        undefined,
        {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
    ...
    if (!highlight) {
      ...
      this.addText(
        '<span style="color: yellow;">' +
          bitcoinString +
          '</span><br /><span style="color: cyan;">Donation</span><br /><span style="color: lime;">Thanks!</span>'
      );
    }
    ...
    var pitch = Math.min(
      maxPitch,
      Math.log(bitcoins + logUsed) / Math.log(logUsed)
    );
